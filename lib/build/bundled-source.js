...
exports.BundledSource = class {
    ...
    return amodroTrace(
      {
        ...
        writeTransform: allWriteTransforms({
          ...
          filterDependencies: (moduleName, deps) => this.filterDeps(moduleName, deps)
        }),
        filterDependencies: (moduleName, deps) => this.filterDeps(moduleName, deps)
      },
      loaderConfig
  ...
  filterDeps(moduleName, deps) {
    let excludedDeps = this.bundler.getExcludedDeps();
    let relevant = excludedDeps.filter(x => !x.origin || x.origin === moduleName);
