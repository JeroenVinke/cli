...
define(['esprima', './lang'], function (esprima, lang) {
    ...
    function removeJSExtension(array) {
        for(var i = 0; i < array.length; i++) {
            if (array[i].slice(-3) === '.js') {
    ...
    function parse(moduleName, fileName, fileContents, options, loaderConfig, context) {
        ...
        if (!options.filterDependencies) {
            options.filterDependencies = function (moduleName, deps) { return deps; };
        }
        ...
        if (moduleDeps.length || moduleList.length) {
            ...
            for (i = 0; i < moduleList.length; i++) {
                ...
                depString = arrayToString(removeJSExtension(options.filterDependencies(moduleName, moduleCall.deps)));
            ...
            if (moduleDeps.length) {
                ...
                depString = arrayToString(removeJSExtension(options.filterDependencies(moduleName, moduleDeps)));
