...
const ExcludedDep = require('./excluded-dep').ExcludedDep;
...
exports.Bundler = class {
  ...
  getExcludedDeps() {
    return this.excludedDeps.concat(this.bundles.reduce((a, b) => a.concat(b.excludedDeps), []));
  }
  ...
  configureDependency(dependency) {
    return analyzeDependency(this.packageAnalyzer, dependency).then(description => {
      ...
      let packageName = loaderConfig.name;
      ...
      // add RequireJS maps based on the browser field in package.json
      let metadata = description.metadata;
      if (metadata && metadata.browser && typeof metadata.browser !== 'string') {
        if (!this.loaderConfig.map) this.loaderConfig.map = {};
        if (!this.loaderConfig.map['*']) this.loaderConfig.map['*'] = {};

        let resolvedMap = {};
        let map = metadata.browser;
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          if (typeof value === 'boolean') {
            this.excludedDeps.push(new ExcludedDep(packageName, key));
          } else {
            resolvedMap[Utils.resolveRelativeModuleId(key, packageName)] = Utils.resolveRelativeModuleId(value, packageName);
          }
        }
        this.loaderConfig.map['*'] = Object.assign({}, resolvedMap, this.loaderConfig.map['*']);
      }

      // register excluded deps defined in a aurelia.json dependency
      // these deps will not be traced and therefore not included in the bundle
      if (loaderConfig.override && loaderConfig.override.excludedDeps) {
        let excludedDeps = loaderConfig.override.excludedDeps;
        let keys = Object.keys(excludedDeps);
        for (let i = 0; i < keys.length; i++) {
          let key = keys[i];
          let value = excludedDeps[key];

          if (typeof(value) === 'boolean') {
            this.excludedDeps.push(new ExcludedDep(packageName, key));
          } else {
            this.excludedDeps.push(new ExcludedDep(packageName, value, key));
          }
        }
      }
