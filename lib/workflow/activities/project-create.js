...
module.exports = class {
  ...
  execute(context) {
    let model = {
      ...
      buildsystem: context.state.buildsystem,
    ...
    return this.ui.clearScreen()
      ...
      .then(answer => {
        if (answer.value === 'yes') {
          let configurator = require(`../../commands/new/buildsystems/${model.buildsystem.id}`);
  ...
  createProjectDescription(model) {
    ...
    text += `    <magenta><bold>Buildsystem: </bold></magenta>${model.buildsystem.displayName}` + os.EOL;
    if (model.loader) {
    ...
    if (model.unitTestRunner) {
      let testRunners = model.unitTestRunner instanceof Array ? model.unitTestRunner : [model.unitTestRunner instanceof Array];
