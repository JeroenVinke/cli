...
exports.Project = class {
  ...
  constructor(ui, directory, model, pack) {
    ...
    this.model = model;
    this.package = pack;
    this.taskDirectory = path.posix.join(directory, 'aurelia_project/tasks');
    this.generatorDirectory = path.posix.join(directory, 'aurelia_project/generators');
    this.aureliaJSONPath = path.posix.join(directory, 'aurelia_project', 'aurelia.json');
  ...
  resolveGenerator(name) {
    let potential = path.posix.join(this.generatorDirectory, `${name}${this.model.transpiler.fileExtension}`);
    return fs.stat(potential).then(() => potential).catch(() => null);
  }

  resolveTask(name) {
    let potential = path.posix.join(this.taskDirectory, `${name}${this.model.transpiler.fileExtension}`);
    return fs.stat(potential).then(() => potential).catch(() => null);
  }
